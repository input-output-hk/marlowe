...
%SJT: would be usweful to say something more about what symbolic parameters mean here: it's essentially that these are parameters that are universally quantified and passed to the solver, I think? They are instantiated with counter-examples if those exist. 
\begin{verbatim}
...
%SJT: i.e. there's no symbolic representation of user defined data types in general?
...
%SJT: need to explain a little more about what the bounds are
...
\noindent
Because working with nested types is much more error prone than working with the original data-types, we used Template Haskell % SJT: nice! Need to add reference.
...
%SJT: a thought: could just give the type here, or cross-refer to an earlier section.
...
%SJT: have I rephrased this right?
