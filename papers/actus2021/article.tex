...
\begin{document}
\title{Standardized crypto-loans on Cardano blockchain\textquotedblright \thanks{Supported by organization x.}}
...
\institute{\\
 \url{http://www.springer.com/gp/computer-science/lncs} \and ABC
...
with a cryptographic signature (Choice owner). As a result, the event
of receiving data from Oracle is treated the same as receiving integer
numbers from user input in other languages
...
See \href{https://github.com/dk14/plutus/tree/ACTUS}{https://github.com/dk14/plutus/tree/ACTUS}
...
\begin{figure} 
\includegraphics[width=0.8\textwidth,natwidth=664,natheight=552]{labs.png}
...
properties of a Marlowe contract. This could be enhanced with Marlowe's
...
tooling which notifies users about all ``Close'' constructs that
can lead to automatic refunds and encourages users to write explicit
...
- Marlowe ACTUS-based implementation tends to rely on ``good old''
matching instead of pooling. While pooling is proven to be superior
to order-book based approaches when it comes to automated market makers,
using it for lending has been shown to be more susceptible to attacks
(for example ``\href{https://www.coindesk.com/harvest-finance-24m-attack-triggers-570m-bank-run-in-latest-defi-exploit}{flash loan}'').
