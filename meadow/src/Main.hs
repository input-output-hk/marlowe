...
insertActionWithNum = F.ffi (J.pack "(function (x, y, z) {var a = document.getElementById('actions'); var r = a.insertRow(); var c1 = r.insertCell(); c1.appendChild(document.createTextNode(x + ' ')); var input = document.createElement('input'); input.type = 'number'; var ch = 'ibox' + a.childNodes.length; input.id = ch; input.value = 0; input.style.setProperty('width', '5em'); input.style.setProperty('display', 'inline'); c1.appendChild(input); c1.appendChild(document.createTextNode(' ' + y)); var c2 = r.insertCell(); var btn = document.createElement('button'); c2.appendChild(btn); btn.appendChild(document.createTextNode('Add action')); btn.style.setProperty('width', '100%'); btn.onclick = function () {Haste.addActionWithNum(z, document.getElementById(ch).value);};})")
