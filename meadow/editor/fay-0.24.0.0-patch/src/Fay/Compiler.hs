...
import           Fay.Compiler.InitialPass        (initialPass, initialPassText)
...
compileToplevelModuleText :: FilePath -> String -> F.Module -> Compile [JsStmt]
compileToplevelModuleText filein text mod@Module{} = do
  cfg <- config id
  ...
  initialPassText filein text
  -- Reset imports after initialPass so the modules can be imported during code generation.
  (hstmts, fstmts) <- startCompileText compileFileWithSource filein text
  return (hstmts++fstmts)
compileToplevelModuleText _ text m = throwError $ UnsupportedModuleSyntax "compileToplevelModuleText" m
