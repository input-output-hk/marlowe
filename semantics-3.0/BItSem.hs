...
data Bounds = Bounds {
    ...
    secretBounds :: Map SecretHash Bound,
...
data Commitment = Commit Party Value
                | Secret Party SecretHash
                deriving (Eq, Ord, Show, Read)

data Contract =
    Null |
    Commits (NonEmpty Commitment) Timeout Contract Contract |
    Pay Party |
    Split (NonEmpty (Value, Contract)) |
...
data Input = Input { inputCommand :: InputCommand
                   ...
                   , inputSecrets :: Map SecretHash SecretValue }
               deriving (Eq, Ord, Show, Read)
...
data State = State { stateChoices :: Map ChoiceId Integer
                   , stateSecrets :: Map SecretHash SecretValue
                   , stateBounds :: Bounds
...
initEnvironment slotNumber Input {..} State {..}
  | M.null $ M.intersection inputChoices stateChoices = Just $ \availableMoney ->
          Environment { envSlotNumber = slotNumber
                      , envChoices = M.union inputChoices stateChoices
                      , envBounds = stateBounds
                      , envOracles = inputOracleValues
                      , envAvailableMoney = availableMoney }
  | otherwise = Nothing
...
reduce env contract = case contract of
    ...
    Commits _ _ _ _ -> Just $ (availableMoney, contract) :| []
    Pay _ -> Just $ (availableMoney, contract) :| []
    Split shares -> let
    ...
    When cases timeout timeoutCont ->
        ...
        then go timeoutCont
        else case find (\(Case obs _) -> evalObservation env obs) cases of
...
contractLifespan contract = case contract of
    Null -> 0
    Commits _ timeout contract1 contract2 ->
        maximum [timeout, contractLifespan contract1, contractLifespan contract2]
    Pay _ -> 0
    Split shares ->   let contractsLifespans = fmap (contractLifespan . snd) shares
...
expireContract slotNumber contract = case contract of
    Null -> Null
    Commits _ timeout _ cont2 ->
        if isExpired slotNumber timeout then go cont2 else contract
    Pay _ -> contract
    Split shares -> Split $ fmap (\(v, c) -> (v, go c)) shares
...
inferActions contract = let
    inner = case contract of
        ...
        Commits{} -> [contract]
        Pay{} -> [contract]
        Split shares -> foldMap (\(_, c) -> inferActions c) (NE.toList shares)
        If{} -> [contract]
        When{} -> []
...
escrow :: Contract
escrow = Commits (Commit alice (Constant 450) :| []) 10
    (When [ Case majorityAgrees (Pay bob)
          , Case majorityDisagrees (Pay alice)]
        90 (Pay alice))
    Null
