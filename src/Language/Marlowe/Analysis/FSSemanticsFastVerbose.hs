...
type Trace = [(Integer, Integer, Integer, Integer)]
...
convertToSymbolicTrace [] = SL.nil 
convertToSymbolicTrace ((lowS, highS, inp, pos):t) =
  (ST.tuple (lowS, highS, getSymValFrom inp, literal pos)) SL..: (convertToSymbolicTrace t)
...
wrapper :: Contract -> STrace -> Symbolic SBool
wrapper c st = do r <- isValidAndFails c
                  return (SM.sJust st .== r)
