...
\begin{itemize}
    ...
    \item We use a \emph{custom implementation of maps and sets} that use lists of tuples and lists of elements respectively. We do this because Isabelle already provides many theorems that are proven for lists, so it considerably facilitates the automation of certain proofs.
\end{itemize}
...
The way we proved that this is the case was by establishing a measurment for the size of a pair of \texttt{Contract} and \texttt{State}. And we defined such measurement as follows:
...
The reason why we need the number of accounts in the \texttt{State} for the measurement is that \texttt{reduceContractStep}, when called with the contract \texttt{Close}, will refund one of the accounts and the contract will remain as \texttt{Close}.

And the reason why we multiply the size of the \texttt{Contract} by two is that the primitive \texttt{Deposit} may increase the number of accounts in the \texttt{State} by one, but it will also reduce the size of \texttt{Contract} by at least one. By multiplying the size of the \texttt{Contract} by two, its decrease compensates the increase in the size of the list of accounts and, thus, it ensures that the measurement will decrease even in that scenario.

\subsubsection{Valid state and positive account preservation}

Since we use lists of pairs of integers as maps in our \texttt{State}, there are some values that are allowed by the type definition of state but, nevertheless, they make no sense given what these lists represent. In particular:

\begin{enumerate}
    \item The lists of tuples are representing a maps, so it does not make sense for two elements of the list to have the same first element (key).
    \item Additionally, we want to minimise the number of ways in which a map can be represented as a list, so we require the first elements of the elements of the list to be sorted ascendingly. This allows us to use standard equality for comparing maps, for example.
    \item We only expect accounts to store positive amounts money, so the second element of the pairs of the list of accounts must be a positive integer. Whenever an account becomes empty we just delete the account from the list.
\end{enumerate}

The two first invariants we define as \texttt{validState}. And the latest we define as follows:
...
And it must be \texttt{True} independently of the \texttt{AccountId} and \texttt{Token}.

We prove that, if the properties are true for a given input \texttt{State}, then the semantics will preserve the properties in the output \texttt{State}.

\subsubsection{Quiescent result}

There is a class of \texttt{Contract}s that will not evolve when passed to the semantics unless a matching \texttt{Input} is passed as well. We call this state \textit{quiescent} and we define it as follows:
...
We prove that if the input \texttt{State} is valid and all accounts are positive, then the result of a transaction is always quiescent.

\subsubsection{Money preservation}

One of the dangers of using smart-contracts is that, a badly written one may lock its funds forever. We want to avoid that feature in Marlowe altogether and, with that aim, we proved that Marlowe account system does not create or destroy any money. By the end of the contract, all the money should be distributed back somehow to a subset of the participants.
