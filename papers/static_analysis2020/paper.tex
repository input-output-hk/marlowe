...
on how those functions are implemented.

However, static analysis can be very computationally expensive, because
it essentially needs to check every possible execution of the contract.
...
The case study presented is that of the optimisation of Marlowe\textquoteright s
...
have no impact on the analysis, then we can remove all but one when
...
use it to model possible executions, and we also ensure that we only
...
For example, in the case of Marlowe\textquoteright s static analysis,
