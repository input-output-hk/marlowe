...
insertActionWithNum = F.ffi (J.pack "(function (x, y, z) {var r = document.getElementById('actions').insertRow(); var c1 = r.insertCell(); c1.appendChild(document.createTextNode(x + ' ')); var input = document.createElement('input'); input.type = 'number'; var ch = 'ibox' + r.childNodes.length; input.id = ch; input.value = 0; input.style.setProperty('width', '5em'); c1.appendChild(input); c1.appendChild(document.createTextNode(' ' + y)); var c2 = r.insertCell(); var btn = document.createElement('button'); c2.appendChild(btn); btn.appendChild(document.createTextNode('Add action')); btn.style.setProperty('width', '100%'); btn.onclick = function () {Haste.addActionWithNum(z, document.getElementById(ch).value);};})")
