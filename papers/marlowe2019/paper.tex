...
%Contracts in Marlowe run on a blockchain, but need to interact with the off-chain world. 
...
It extends the Bitcoin model by allowing transaction outputs to hold a \emph{Dataum}.
...
with the particular Marlowe contract in the \emph{Datum}, and protected by the Marlowe validator script.
...
Cardano extends its ledger rules to support \emph{forging} of custom currencies and tokens.%effectively embedding support of Ethereum ERC-20 and ERC-721 contracts. 
...
%    \vspace*{-0.25in}
    \includegraphics[scale=0.18]{hole_options_2.png}
\end{wrapfigure}
